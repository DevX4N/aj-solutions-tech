...
// Canvas grid whose cells light up around the pointer. Adapted to run as a
// background layer: pointer-events:none, listeners on window (so it tracks even
// under the hero content), brand-colored, and disabled under reduced motion /
// non-fine pointers. Based on the provided CursorGrid component.
...
export default function CursorGrid({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e) => {
      ...
      const [x, y] = toLocal(e)
      if (x < 0 || y < 0 || x > w || y > h) return // only while over the hero
      energize(x, y)
      wake()
    }

    const onPointerDown = (e) => {
      if (!propsRef.current.clickPulse || e.pointerType === 'touch') return
      const [x, y] = toLocal(e)
